...
const AddToCartBtn = ({
  ...
  const handleAddToCart = () => {
    if (product) {
      addToCart(product);
      toast.success(`${product?.name.substring(0, 10)} Đã thêm 1 sản phẩm!`);
    } else {
      toast.error("Sản phẩm không thể thêm!");
    }
  };
  ...
  const handleDeleteProduct = () => {
    if (existingProduct) {
      if (existingProduct?.quantity > 1) {
        ...
        toast.success(
          `${product?.name.substring(0, 10)} Đã giảm 1 sản phẩm!`
        );
      } else {
        toast.error("Bạn không thể giảm số lượng sản phẩm!");
      }
    } else {
    }
  };
